...
(function() {
    ...
    try {
      ...
      if (partnerData && partnership) {
        ...
        if (perms.allow_tasks) {
          ...
          // Repeating tasks — only save tasks created by this user (not tasks created by partner)
          // This prevents re-activating tasks the other user deleted
          for (const t of (partnerData.repeating || []).filter(t => t.createdBy === userId)) {
  ...
  if (req.method === 'POST' && url === '/api/task/update') {
    ...
    if (!userId) { json(res, { error: 'Unauthorized' }, 401); return; }
    try {
      const { taskId, name, desc, pts, requireProof, visibleTo, updateVisibility, expiresOn, updateExpiry } = await parseBody(req);
      if (updateExpiry) {
        await db.query(`UPDATE tasks SET expires_on=$1 WHERE id=$2 AND (created_by=$3 OR owner_id=$3)`,
          [expiresOn, taskId, userId]);
